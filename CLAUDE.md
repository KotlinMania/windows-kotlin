# Claude Code Project Instructions — windows-kotlin

## Project Overview

This is **windows-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`windows`](https://crates.io/crates/windows).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/windows/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/windows>
